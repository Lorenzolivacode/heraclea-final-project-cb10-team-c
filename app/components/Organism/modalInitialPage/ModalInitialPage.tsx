...
function InitialPagemodal() {
	...
	const pages = [
		...
		{ title: "Scopri", description: "Scopri il museo, il teatro, la necropoli", video: "/search.mp4" },
		{ title: "Acquista", description: "Acquista il tuo biglietto di ingresso", video: "/tickets.mp4" },
		{ title: "Percorsi", description: "Scegli il tuo percorso di visita", video: "/trekking.mp4" },
		{
			title: "Eventi",
	...
	const params = ["pagina_0", "pagina_1", "pagina_2", "pagina_3", "pagina_4", "pagina_5"];
	...
	const handleTouchEnd = (e: React.TouchEvent) => {
		...
		if (touchStartX.current - touchEndX > swipeThreshold && currentPage < pages.length - 1) {
			// Swipe sinistra -> pagina successiva
			setCurrentPage(currentPage + 1);
			setSelectedPage(params[currentPage + 1]);
			scrollToPage(currentPage + 1);
		} else if (touchEndX - touchStartX.current > swipeThreshold && currentPage > 0) {
	...
	return (
		<Modal isVisible={isModalVisible} onClose={closeModal}>
			...
			>
				<main className={`${style.main} ${currentPage === 0 ? style.initialPage : ""}`}>
